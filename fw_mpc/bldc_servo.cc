...
#include "fw_mpc/bldc_servo_position.h"
#include "fw_mpc/foc.h"
#include "fw_mpc/moteus_math.h"
#include "fw_mpc/moteus_hw.h"
#include "fw_mpc/sensorless_observer.h"
#include "fw_mpc/stm32g4_adc.h"
#include "fw_mpc/thermistor.h"
#include "fw_mpc/torque_model.h"
#include "fw_mpc/parameter_estimator.h"
#include "fw_mpc/mpc_kalman.h"

#if defined(TARGET_STM32G4)
#include "fw_mpc/stm32g4_async_uart.h"
#else
...
//    velocity_filter_ = ExponentialFilter(rate_config_.pwm_rate_hz, 0.01f);
	// --- LAM MUOT BO LOC VAN TOC --- (SMO)
	// Tang thoi gian loc (giam tan so cat) de lam muot tin hieu van toc tu SMO
    velocity_filter_ = ExponentialFilter(rate_config_.pwm_rate_hz, 0.01f);
    ...
    // --- KHOI TAO & DONG BO RLS ---
	float current_temp_C = config_.enable_motor_temperature
		? status_.filt_motor_temp_C
		: status_.filt_fet_temp_C;

	// Tranh ADC chua san sang tra ve NaN
	if (std::isnan(current_temp_C)) {
		current_temp_C = 25.0f; // Mac dinh 25 do C neu chua co du lieu
	}

	const float init_R = motor_.resistance_ohm;

	// Tranh chia cho 0 neu poles chua kip load tu JSON
	const float init_flux = (motor_.Kv > 0.0f && motor_.poles > 0)
	...
	if (!rls_initialized_) {
		...
		parameter_estimator_.ApplyNewCalibration(init_R, init_Lq, init_flux, current_temp_C);
	}
	// --------------------------------------------------
	// --- Khoi tao lai MPC khi Config thay doi ---
	    mpc_kalman_.Init(rate_config_.period_s);
  }
  ...
  void PollMillisecond() {
    ...
    // =======================================================
	// BAT LED CHON MODE TAI VONG LAP 1kHz (SMO)
	// =======================================================
	if (config_.enable_sensorless && force_encoder_disconnect_) {
		power_led_.write(1); //Tat LED
	}
...
private:
	...
	if (data->synthetic_theta) {
	  electrical_theta = WrapZeroToTwoPi(
		  ...
		  motor_.poles * 0.5f * k2Pi);
			...
			// 1. KIEM TRA PHAN CUNG VA PHAN NHANH MODE
			bool encoder_physically_ok = position_.position_relative_valid &&
							  position_.theta_valid &&
							  (position_.error == MotorPosition::Status::kNone);
	        ...
	        if (config_.enable_sensorless) {
	            // SENSORLESS MODE: Ep ngat ket noi vat ly khi toc do cao
	            if (smo_mech_hz_abs > kSensorlessSwitchHz + 0.2f) {
	                force_encoder_disconnect_ = true;
	            } else if (smo_mech_hz_abs < kSensorlessSwitchHz - 0.2f) {
	                force_encoder_disconnect_ = false;
	            }
	        } else {
	            // NORMAL MODE: Tuan thu tuyet doi Encoder
	            force_encoder_disconnect_ = false;
	        }

	        // Chot trang thai Encoder
	        bool encoder_ok = encoder_physically_ok && !force_encoder_disconnect_;

			float ref_vel_hz = (!encoder_ok) ? smo_mech_hz_abs : ((sensorless_alpha_blend_ >= 1.0f) ? smo_mech_hz_abs : enc_mech_hz_abs);
			const float fading_band = 0.5f;
			float alpha = 0.0f;

			bool is_calibrating = (!is_torque_constant_configured() || motor_.resistance_ohm == 0.0f || motor_.poles == 0 || status_.mode == kMeasureInductance || status_.mode == kCalibrating);
			...
			if (!is_calibrating) {
	            if (!config_.enable_sensorless) {
	                ...
	                if (!encoder_ok) {
	                    if (smo_mech_hz_abs >= kSensorlessSwitchHz) alpha = 1.0f;
	                } else {
	                    if (ref_vel_hz >= kSensorlessSwitchHz) alpha = 1.0f;
	                    else if (ref_vel_hz > (kSensorlessSwitchHz - fading_band)) alpha = (ref_vel_hz - (kSensorlessSwitchHz - fading_band)) / fading_band;
	                }
	            }
			}
			...
			// 5. CACH LY TUYET DOI VI TRI AO
			if (alpha >= 1.0f && !encoder_ok) {
			    float dt_s = rate_config_.period_s;
			    float d_pos = active_smooth_out_hz * dt_s;
			    active_position_ += d_pos;

			    // Cap vi tri ao cho he thong (MPC & PID)
			    status_.position = active_position_;

			    active_position_relative_raw_ += static_cast<int64_t>(d_pos * 4294967296.0f);
			} else {
			    active_position_ = position_.position;
			    status_.position = active_position_;
  ...
  void ISR_CalculateCurrentState(const SinCos &sin_cos) MOTEUS_CCM_ATTRIBUTE {
    ...
    DAC1->DHR12R1 = static_cast<uint32_t>(dq.d * 400.0f + 2048.0f);
		...
		// - Encoder angle de khoi tao
    			float safe_poles = (motor_.poles > 0) ? motor_.poles : 14.0f;
				...
				// --- KHOI PHUC DONG BO O TOC DO THAP ---
				// CHI dong bo khi mach tin tuong Encoder (alpha < 1) va Encoder con song
				bool encoder_ok = position_.position_relative_valid && position_.theta_valid && (position_.error == MotorPosition::Status::kNone);

				if (sensorless_alpha_blend_ < 1.0f && encoder_ok) {
					if (std::abs(active_mech_hz_) < kSensorlessSwitchHz) {
  ...
  bool ISR_IsOutsideLimits() {
    return ((!std::isnan(position_config_.position_min) &&
    		 status_.position < position_config_.position_min) || // Doi thanh status_.position
            (!std::isnan(position_config_.position_max) &&
             status_.position > position_config_.position_max));  // Doi thanh status_.position
  }
  ...
  void ISR_DoControl(const SinCos &sin_cos,
	...
	// Dam bao nhiet do khong bi NaN trong luc chay Standby
	if (std::isnan(current_temp_C)) {
		current_temp_C = 25.0f;
	}

	// chi chay update ma tran khi o che do dong kin
	if (status_.mode == kCurrent || status_.mode == kPosition || status_.mode == kStayWithinBounds || status_.mode == kVoltageDq) {
		...
		float poles = (motor_.poles > 0) ? motor_.poles : 14.0f;
//		float we = position_.velocity / motor_position_->config()->rotor_to_output_ratio * poles * 0.5f * k2Pi;

		// Dong nhat van toc cap cho RLS voi van toc he quy chieu d-q (Da qua Hybrid Blend)
		float we = active_mech_hz_ * poles * 0.5f * k2Pi;
		if (std::isnan(we)) we = 0.0f;
		if (std::isnan(id)) id = 0.0f;
		if (std::isnan(iq)) iq = 0.0f;

		parameter_estimator_.Update(id, iq, ud, uq, we, rate_config_.period_s, current_temp_C);
	}
	else if (status_.mode == kStopped) {
		// Chay Standby de theo doi su phan tan nhiet va cap nhat thong so goc luc dong co dung
		parameter_estimator_.UpdateStandby(rate_config_.period_s, current_temp_C);
	}

	// DUA CAP NHAT TELEMETRY RA NGOAI DE GRAPH
  ...
  void ISR_DoCurrent(const SinCos &sin_cos, float i_d_A_in, float i_q_A_in,
    ...
    // Neu dang chay 100% Sensorless thi bo qua loi mat goc tu Encoder (SMO)
            ...
            MOTEUS_CCM_ATTRIBUTE {
              ...
              if (!std::isnan(position_config_.position_max) &&
                ...
                return std::make_pair(
                    in * std::max(0.0f, 1.0f - (status_.position - // Doi thanh status_.position
              ...
              if (!std::isnan(position_config_.position_min) &&
                ...
                return std::make_pair(
                    in * std::max(0.0f, 1.0f - (position_config_.position_min -
                    							   status_.position) / // Doi thanh status_.position
                                                   config_.position_derate),
                    errc::kLimitPositionBounds);
              }
...
//              }
              ...
              // Su dung van toc Hybrid (status_.velocity) thay cho encoder thuan (SMO)
...
//    const float we = motor_position_config()->output.sign * position_.velocity / motor_position_->config()->rotor_to_output_ratio * motor_.poles * 0.5f * k2Pi;
    ...
    // SMO + RLS vao FOC
    // Dung active_mech_hz_ cho bo bu cheo (Decoupling) de khong bom nhieu thang vao Vd, Vq lam sap SMO
	const float we_ctrl = active_mech_hz_ * motor_.poles * 0.5f * k2Pi;

	// Trung khop he so Tuning cua Sensorless va Encoder
...
//    }
    ...
    // Neu dang chay full SMO thi bo qua loi mat goc
        if (!position_.theta_valid && sensorless_alpha_blend_ < 1.0f) {
          status_.mode = kFault;
          status_.fault = errc::kThetaInvalid;
          return;
...
//    }
    ...
    // Bo qua loi mat ket noi Encoder neu dang bam dinh SMO
    ...
    //Cat nguon du lieu Encoder cua PID (SMO)
    //Dung status_.velocity (da bi ghi de bang Hybrid) thay vi position_.velocity phan cung
    ...
    // SUA: Dung active_position_relative_raw_ thay vi position_.position_relative_raw
...
//             feedforward_Nm + inertia_feedforward_Nm);
    ...
    // --- Tich hop MPC-Kalman hoac dung PID mac dinh ---
    float unlimited_torque_Nm = 0.0f;

	if (config_.enable_mpc) {
		// Doi don vi sang he SI chuan cho thuat toan MATLAB (rad, rad/s)
		float w_m_meas = status_.velocity * k2Pi;         // rad/s (da qua loc Hybrid SMO)
		float th_m_meas = status_.position * k2Pi;        // rad
		float w_ref = velocity_command * k2Pi;            // rad/s
		float th_ref = status_.control_position * k2Pi;   // rad

		// Phat hien y dinh cua nguoi dung (Co phai lenh d pos nan khong?)
		bool is_vel_only = std::isnan(data->position);

		// Do thoi gian bat dau
		uint32_t mpc_start_cycle = DWT->CYCCNT;

		// Cap nhat MPC voi co is_vel_only
		auto mpc_out = mpc_kalman_.Update(w_m_meas, th_m_meas, w_ref, th_ref, max_torque_Nm, is_vel_only);

		// Do thoi gian ket thuc va tinh toan
		uint32_t mpc_end_cycle = DWT->CYCCNT;
		uint32_t current_cycles = mpc_end_cycle - mpc_start_cycle;

		// CHI CAP NHAT KHI MPC THUC SU GIAI TOAN (Loc bo cac chu ky bi skip co cycles < 500)
		if (current_cycles > 500) {
			status_.mpc_compute_cycles = current_cycles;
			// STM32G4 cua Moteus thuong chay o xung nhip 170 MHz (170 cycles = 1 us)
			status_.mpc_compute_time_us = (float)current_cycles / 170.0f;
		}

		unlimited_torque_Nm = mpc_out.torque_cmd_Nm * config_.pid_position.sign;

			// Cap nhat len Status de gui du lieu Telemetry ve may tinh (Graph)
			status_.mpc_estimated_omega = mpc_out.state.omega_rad_s;
			status_.mpc_estimated_theta = mpc_out.state.theta_rad;
			status_.mpc_estimated_torque = mpc_out.state.torque_est_Nm;
			status_.mpc_torque_command = mpc_out.torque_cmd_Nm;
			status_.mpc_iq_reference = mpc_out.iq_ref;
			status_.mpc_id_reference = mpc_out.id_ref;
		} else {
            // Thuat toan PID Position mac dinh cua Moteus
            unlimited_torque_Nm =
                (pid_position_.Apply(
                     (static_cast<int32_t>((active_position_relative_raw_ -
                                            *status_.control_position_raw) >>
...
//          position_.sources[commutation_source].filtered_value / cpr;
      ...
      // ================= SUA LOI RO RI CONG HUONG (SMO) =================
      float commutation_position = 0.0f;
      bool encoder_physically_ok = position_.position_relative_valid && position_.theta_valid && (position_.error == MotorPosition::Status::kNone);
      bool encoder_ok = encoder_physically_ok && !force_encoder_disconnect_;

      // Neu dang bam 100% SMO ma dut/bi ep ngat Encoder
      if (sensorless_alpha_blend_ >= 1.0f && !encoder_ok) {
          // Dung active_position_ (bien nuoi goc ao) thay vi status_.position da bi ep bang 0
          commutation_position = active_position_;
          commutation_position -= std::floor(commutation_position); // Lay phan thap phan 0.0 -> 1.0
      } else {
          const float cpr = static_cast<float>(pos_config->sources[commutation_source].cpr);
          commutation_position = position_.sources[commutation_source].filtered_value / cpr;
      }
      // ============================================================
...
//    }();
	  ...
	  // Gioi han chan bien (Boundary Check) (SMO)
	  const auto target_position =
	          [&]() MOTEUS_CCM_ATTRIBUTE -> std::optional<float> {
	        if (!std::isnan(data->bounds_min) &&
	            status_.position < data->bounds_min) {
	          return data->bounds_min;
	        }
	        if (!std::isnan(data->bounds_max) &&
	            status_.position > data->bounds_max) {
	          return data->bounds_max;
	        }
	        return {};
	      }();
...
//    ==========
  ...
  // BIEN DE LUU TRANG THAI HYBRID (SMO)
  float sensorless_alpha_blend_ = 0.0f;
  float active_mech_hz_ = 0.0f;
  int64_t active_position_relative_raw_ = 0; 	// bien de nuoi PID Position
  float active_position_ = 0.0f; 				// Bien doc lap de nuoi goc ao
  bool force_encoder_disconnect_ = false; 		// Co ep ngat tin hieu Encoder
  DigitalOut power_led_{g_hw_pins.power_led, 0};// Mac dinh 0 la Sang

  // --- Khai bao MPC Kalman ---
  MPCKalmanEstimator mpc_kalman_;
