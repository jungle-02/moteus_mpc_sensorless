...
#include "fw_mpc/error.h"
#include "fw_mpc/measured_hw_rev.h"
#include "fw_mpc/pid.h"
#include "fw_mpc/simple_pi.h"
...
struct BldcServoStatus {
  ...
  float cos = 0.0f;
	...
	// Uoc luong vi tri tu sensorless observer
  ...
  // MPC+Kalman Filter outputs
   float mpc_estimated_omega = 0.0f;      // Estimated motor speed [rad/s]
   float mpc_estimated_theta = 0.0f;      // Estimated electrical angle [rad]
   float mpc_estimated_torque = 0.0f;     // Estimated load torque [Nm]
   float mpc_torque_command = 0.0f;       // MPC commanded torque [Nm]
   float mpc_iq_reference = 0.0f;         // MPC Iq reference [A]
   float mpc_id_reference = 0.0f;         // MPC Id reference [A]
   uint16_t mpc_status = 0;               // MPC status flags
   uint32_t mpc_compute_cycles = 0;       // Dem so chu ky CPU ton cho MPC
   float mpc_compute_time_us = 0.0f;      // Thoi gian chay MPC (microseconds)
  ...
  template <typename Archive> void Serialize(Archive *a) {
    ...
    //Serial MPC
     a->Visit(MJ_NVP(mpc_estimated_omega));
	 a->Visit(MJ_NVP(mpc_estimated_theta));
	 a->Visit(MJ_NVP(mpc_estimated_torque));
	 a->Visit(MJ_NVP(mpc_torque_command));
	 a->Visit(MJ_NVP(mpc_iq_reference));
	 a->Visit(MJ_NVP(mpc_id_reference));
	 a->Visit(MJ_NVP(mpc_status));
	 a->Visit(MJ_NVP(mpc_compute_cycles));
	 a->Visit(MJ_NVP(mpc_compute_time_us));
...
struct BldcServoConfig {
  ...
  template <typename Archive> void Serialize(Archive *a) {
    ...
    a->Visit(MJ_NVP(enable_mpc)); // bat MPC
	a->Visit(MJ_NVP(enable_sensorless)); //(mode (SMO))
  }
