...
def run_triangle_velocity_test():
    ...
    start_msg = f"OK. Triangle script loaded."
    pub_socket.send_string(json.dumps({"type": "log", "msg": start_msg}))
    print(f">> {start_msg}")

    # --- 2. Cau hinh tham so song Tam giac (Triangle) ---
    ...
    while True:
        ...
        # Van toc lenh duoc quy hoach theo hinh tam giac
        # Cong thuc: v(t) = (2A/pi) * arcsin(sin(2*pi*f*t))
        velocity_command = (2 * A / math.pi) * math.asin(math.sin(2 * math.pi * f * t_simulated))
...
if __name__ == "__main__":
    try:
        ...
        sock.send_string(json.dumps({"type": "log", "msg": "Triangle script stopped."}))
        sock.close()
        ctx.term()
        sys.exit(0)