...
#include <cstdint>
#include <cmath>
#include "fw_smo/moteus_math.h"

namespace moteus {

// MACRO TOAN HOC TOI UU CHO FLOAT
...
// SENSORLESS OBSERVER
// SU DUNG ASMO + PLL DAI SO DE UOC LUONG GOC VA TOC DO
class SensorlessObserver {
 public:
  struct Config {
    // CAC HE SO CHINH CUA SMO
    float chi = 15.0f;
    float a = 0.02f;
  ...
  // SET CAU HINH VA CAP NHAT CAC HE SO PHU THUOC dt
  ...
  // HAM UPDATE CHINH - DUOC GOI MOI CHU KY PWM
  void Update(float i_d_A, float i_q_A,
    ...
    // ============================================================
    // TRANG THAI KHONG ACTIVE (MOTOR KHONG CHAY)
    // CHI GIAM TOC TU NHIEN VA RESET CAC BO LOC
    // ============================================================
    if (!is_active) {
    ...
    // ============================================================
    // BIEN DOI SANG HE TOA DO ALPHA-BETA
    // ============================================================
    ...
    // ============================================================
    // TINH SAI SO DONG DIEN VA HAM TRUOT (SLIDING MODE)
    // ============================================================
    ...
    // LUC KE0 CHONG UOC LUONG SAI KHI QUA GOC TOA DO
  ...
  // RESET OBSERVER NHUNG GIU TRANG THAI ALIGN
  void Reset() {
  ...
  // ALIGN LAN DAU TU ENCODER
  void AlignOnce(float encoder_theta) {
    if (!state_.is_aligned) {
 ...
 private:
  // TIEN TINH CAC HE SO PHU THUOC dt
