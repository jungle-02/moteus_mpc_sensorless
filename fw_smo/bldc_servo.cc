...
#include "fw_smo/bldc_servo_position.h"
#include "fw_smo/foc.h"
#include "fw_smo/moteus_math.h"
#include "fw_smo/moteus_hw.h"
#include "fw_smo/sensorless_observer.h"
#include "fw_smo/stm32g4_adc.h"
#include "fw_smo/thermistor.h"
#include "fw_smo/torque_model.h"
#include "fw_smo/parameter_estimator.h"

#if defined(TARGET_STM32G4)
#include "fw_smo/stm32g4_async_uart.h"
#else
  ...
  void ISR_MaybeChangeMode(CommandData *data) MOTEUS_CCM_ATTRIBUTE {
    ...
    case kBrake: {
      ...
      case kBrake: {
        ...
        // ========================================================
		// DONG BO GOC SMO TU ENCODER KHI BAT DAU CHAY
		// ========================================================
		// chuyen vi tri output -> vi tri rotor (so vong co)
		const float rotor_mech_revs = position_.position / motor_position_->config()->rotor_to_output_ratio;
		// chi align 1 lan duy nhat
		sensorless_observer_.AlignOnce(rotor_mech_revs);
		// ========================================================
  ...
  void ISR_DoCurrent(const SinCos &sin_cos, float i_d_A_in, float i_q_A_in,
    ...
    if (!config_.voltage_mode_control) {
    	const float denorm_d_V =
    	          ...
    	          we * rls_Lq_hat_ * i_q_A; // Sua status_.rls_Lq_hat thanh rls_Lq_hat_

    	      const float denorm_q_V =
    	          pid_q_.Apply(status_.q_A, i_q_A, rate_config_.rate_hz) +
    	          i_q_A * config_.current_feedforward * rls_R_hat_ +
    	          we * rls_Ld_hat_ * i_d_A + // Sua status_.rls_Ld_hat thanh rls_Ld_hat_
