...
#include "../fw/asmo.h" // ASMO observer
...
public:
  // Khai bao bien, thong so cho ASMO
  AsmoObserver asmo_;
  AsmoObserver::Config asmo_config_;
  float last_cmd_vel_ = 0.0f;
  ...
  void Start() {
    ...
    // === [CAU HINH TU DONG CHO ASMO] ===

    // 1. Tinh chu ky dt (tu tan so PWM thuc te)
    if (config_.pwm_rate_hz > 0) {
      asmo_config_.dt = 1.0f / static_cast<float>(config_.pwm_rate_hz);
    } else {
      asmo_config_.dt = 1.0f / 40000.0f; // Mac dinh
    }

    // 2. Lay Rs (tu ket qua calibrate da luu trong flash)
    asmo_config_.Rs = motor_.resistance_ohm;

    // 3. Tinh Ls (Suy ra tu bo dieu khien dong dien: Ls = Rs * Kp / Ki)
    // Day la cach duy nhat de lay Ls neu khong nhap tay, vi Moteus dung nguyen
    // ly triet tieu cuc.
    if (config_.pid_dq.ki > 1e-5f) {
      asmo_config_.Ls =
          asmo_config_.Rs * (config_.pid_dq.kp / config_.pid_dq.ki);
    } else {
      asmo_config_.Ls = 0.0001f; // Gia tri an toan neu chua tune PID
    }

    // 4. Cac tham so khac
    asmo_config_.mode = 1; // Sine mode
    // =========================================
  }
  ...
  void Command(const CommandData &data) {
    // Lay van toc dat
    if (std::isfinite(data.velocity)) {
      last_cmd_vel_ = data.velocity;
    }
...
private:
  ...
  void ISR_DoTimer() __attribute__((always_inline)) MOTEUS_CCM_ATTRIBUTE {
    ...
    // #############################################################
    // ### BAT DAU THUAT TOAN RLS UOC LUONG R ###
    // #############################################################

    if (current_control()) {
      // --- 1. Lay gia tri tho (Raw values) ---
      ...
      // --- 2. Lay hang so dong co ---
      const float R_calibrated = motor_.resistance_ohm;
      const float Kp_calibrated = config_.pid_dq.kp;
      const float Ki_calibrated = config_.pid_dq.ki;
      const float L_calibrated =
          (std::abs(Ki_calibrated) > 1e-6f)
              ? (R_calibrated * Kp_calibrated / Ki_calibrated)
              : 0.000020f;
      const float Ld = L_calibrated;

      // lambda_m (V.s/rad) = v_per_hz (V/Hz) / 2*Pi ---
      const float lambda_m = v_per_hz_ / k2Pi;
      // -------------------------------------------------------------

      float phi_raw = 0.0f;
      float y_raw = 0.0f;
      const float activation_threshold = 1.0f; // Nguong 1.0A

      // --- 3. TU DONG CHON MO HINH (D-axis hay Q-axis) ---

      if (std::abs(Iq_real) > std::abs(Id_real) &&
          std::abs(Iq_real) > activation_threshold) {
        // --- DANG O CHE DO Q-AXIS (Torque Control 'f...') ---
        // y = Vq - (we*Ld*Id + we*lambda_m)
        // phi = Iq
        phi_raw = Iq_real;
        y_raw = Vq - (we * Ld * Id_real + we * lambda_m);

      } else if (std::abs(Id_real) > std::abs(Iq_real) &&
                 std::abs(Id_real) > activation_threshold) {
        // --- DANG O CHE DO D-AXIS (Heat Mode 'c...') ---
        // Dong co bi khoa -> we ~ 0

        // Dien ap roi
        const float V_offset_drop = 0.23f;
        // y = Vd
        // phi = Id

        phi_raw = Id_real;

        // Tinh thanh phan V_offset
        const float v_offset_signed =
            (Id_real >= 0.0f) ? V_offset_drop : -V_offset_drop;

        // Van tru thanh phan BEMF (nho) cua truc D de tang do chinh xac
        y_raw = Vd + (we * Ld * Iq_real) -
                v_offset_signed; // Vd = R*Id + Ld*dId/dt - we*Lq*Iq -
                                 // V_offset_drop. Bo qua Lq

      } else {
        // Khong du dong dien, khong chay RLS
        phi_raw = 0.0f;
        y_raw = 0.0f;
      }

      // --- 4. Ap dung bo loc thong thap (LPF) ---
      rls_y_filt_ = (1.0f - rls_alpha_) * rls_y_filt_ + rls_alpha_ * y_raw;
      rls_phi_filt_ =
          (1.0f - rls_alpha_) * rls_phi_filt_ + rls_alpha_ * phi_raw;

      // --- 5. Hang rao an toan (dung phi da loc) ---
      if (std::abs(rls_phi_filt_) > activation_threshold) {
        // --- 6. Chay RLS voi du lieu da loc ---
        const float phi = rls_phi_filt_;
        const float y = rls_y_filt_;

        const float K_numerator = rls_P_ * phi;
        const float K_denominator = rls_lambda_ + phi * K_numerator;

        if (std::abs(K_denominator) > 1e-6f) {
          const float K = K_numerator / K_denominator;
          const float error = y - phi * rls_R_hat_;

          rls_R_hat_ = rls_R_hat_ + K * error;
          rls_P_ = (rls_P_ - K * phi * rls_P_) / rls_lambda_;
          ...
          rls_R_hat_ =
              Limit(rls_R_hat_, 0.01f, 0.2f); // Gioi han R (0.01 - 0.2 Ohm)
        }
      }
    }
  ...
  void ISR_DoTimerLowerPriority()
    ...
    // THUC THI BO ASMO
    // 1. Chuyen doi he toa do DQ -> Alpha Beta
    // Bien doi nguoc dien ap (DQ -> AlphaBeta)
    InverseDqTransform idq_v(sin_cos, control_.d_V, control_.q_V);

    // Bien doi nguoc dong dien (DQ -> AlphaBeta)
    InverseDqTransform idq_i(sin_cos, status_.d_A, status_.q_A);

    // 2. Tinh bien do A (rpm) tu lenh van toc (turns/s)
    float A_input = std::abs(last_cmd_vel_) * 60.0f;

    // 3. Cap nhat
    asmo_.Update(asmo_config_, idq_v.a, idq_v.b, idq_i.a, idq_i.b, A_input);

    // 4. Xuat ket qua
    status_.asmo_w_hat = asmo_.state().we_hat;
    status_.asmo_theta_hat = asmo_.state().theta_hat;
  ...
  void ISR_DoCalibrating() {
    ...
    // --- KHOI TAO BIEN RLS ---
    rls_R_hat_ = motor_.resistance_ohm; // Bat dau bang gia tri da hieu chuan
    status_.rls_R_hat = rls_R_hat_;     // Cap nhat status
    rls_P_ = 100.0f;                    // Reset hiep phuong sai ve gia tri lon
    // --------------------------
  }
  ...
  void ISR_DoCurrent(const SinCos &sin_cos, float i_d_A_in, float i_q_A_in,
    ...
    if (!config_.voltage_mode_control) {
      ...
      const float denorm_q_V =
          ...
          (feedforward_velocity_rotor * config_.bemf_feedforward * v_per_hz_);
      ...
      auto [d_V, q_V, final_limit_code] = limit_to_max_voltage(
          i_d_A * rls_R_hat_,
          i_q_A * rls_R_hat_ + (feedforward_velocity_rotor *
                                config_.bemf_feedforward * v_per_hz_),
          limit_code);
  ...
  float rls_R_hat_ = 0.0f; // R_hat (theta) - Gia tri R uoc luong
  float rls_P_ = 100.0f;   // Ma tran hiep phuong sai P (1x1) - khoi tao lon
  const float rls_lambda_ = 0.999f; // He so lang quen (0.99 - 0.999)

  // --- CAC BIEN LOC ---
  float rls_y_filt_ = 0.0f;
  float rls_phi_filt_ = 0.0f;
  // He so loc (alpha) - 0.01 la kha cham, tot cho viec loc R
  const float rls_alpha_ = 0.01f;
  // ---------------------------------
