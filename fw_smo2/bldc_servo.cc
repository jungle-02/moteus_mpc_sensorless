  ...
  void ISR_DoTimerLowerPriority()
	...
	if (data->synthetic_theta) {
	  electrical_theta = WrapZeroToTwoPi(
		...
		// KIỂM TRA TÌNH TRẠNG SỐNG CỦA ENCODER
		bool encoder_ok = position_.position_relative_valid &&
		...
		// Nếu mất Encoder, buộc phải dùng tham chiếu tốc độ từ SMO
		...
		bool is_calibrating = (!is_torque_constant_configured() ||
							   motor_.resistance_ohm == 0.0f ||
							   motor_.poles == 0 ||
							   status_.mode == kMeasureInductance ||
							   status_.mode == kCalibrating);

		// 1. TÍNH TOÁN ALPHA (Bảo vệ Hot-Swap)
		if (!is_calibrating) {
			if (!encoder_ok) {
				// CỨU HỘ: Đứt dây Encoder! Nếu đang ở dải tốc độ cao -> ÉP chạy Full SMO.
				// Ngược lại nếu tốc độ thấp sẽ chấp nhận Fault để bảo vệ an toàn.
				if (smo_mech_hz_abs >= kSensorlessSwitchHz) {
					alpha = 1.0f;
				}
			} else {
				// BÌNH THƯỜNG: Vận hành Hybrid
		...
		// 2. BLEND GÓC ĐIỆN TỪ (Cho Vòng lặp Dòng điện)
		...
		// 3. BLEND VẬN TỐC (Cho Vòng lặp Vận tốc)
		...
		// 4. KIỂM SOÁT VỊ TRÍ TUYỆT ĐỐI (Sinh Encoder Ảo khi bị rút dây)
		if (alpha >= 1.0f && !encoder_ok) {
			// Tích phân Vận tốc SMO thành góc ảo để nuôi PID Position không bị crash
			float dt_s = rate_config_.period_s;
			float d_pos = active_smooth_out_hz * dt_s;
			status_.position += d_pos;
			active_position_relative_raw_ += static_cast<int64_t>(d_pos * 4294967296.0f); // 2^32
		} else {
			// Bình thường: Dùng nguyên bản Encoder để triệt tiêu trôi dạt (Drift)
  ...
  void ISR_CalculateCurrentState(const SinCos &sin_cos) MOTEUS_CCM_ATTRIBUTE {
    ...
    DAC1->DHR12R1 = static_cast<uint32_t>(dq.d * 400.0f + 2048.0f);
		...
		// - Encoder angle de khoi tao
    			float safe_poles = (motor_.poles > 0) ? motor_.poles : 14.0f; // Tránh truyền 0 cực
    				sensorless_observer_.Update(
                        ...
                        rls_R_hat_,
                        rls_L_hat_,
                        safe_poles,       // Sử dụng safe_poles thay vì motor_.poles trực tiếp
    					old_sin,
                        old_cos,
                        is_torque_on
                    );
				...
				// CHỈ đồng bộ khi mạch tin tưởng Encoder (alpha < 1) và Encoder còn sống
...
//		float we = position_.velocity / motor_position_->config()->rotor_to_output_ratio * poles * 0.5f * k2Pi;
		...
		// Đồng nhất vận tốc cấp cho RLS với vận tốc hệ quy chiếu d-q (Đã qua Hybrid Blend)
...
//              }
              ...
              // Sử dụng vận tốc Hybrid (status_.velocity) thay cho encoder thuần (SMO)
...
//    }
    ...
    // BYPASS: Nếu đang chạy full SMO thì bỏ qua lỗi mất góc
        if (!position_.theta_valid && sensorless_alpha_blend_ < 1.0f) {
          status_.mode = kFault;
          status_.fault = errc::kThetaInvalid;
          return;
...
//          position_.sources[commutation_source].filtered_value / cpr;
      ...
      float commutation_position = 0.0f;
            ...
            // Nếu đang bám 100% SMO mà đứt Encoder, dùng vị trí mô phỏng thay vì vị trí chết của Encoder
            if (sensorless_alpha_blend_ >= 1.0f && !encoder_ok) {
                commutation_position = status_.position;
                commutation_position -= std::floor(commutation_position); // Chặt lấy phần thập phân 0.0 -> 1.0
            } else {
...
//    }();
	  ...
	  // Sửa nốt giới hạn chặn biên (Boundary Check) (SMO)
	  const auto target_position =
	          [&]() MOTEUS_CCM_ATTRIBUTE -> std::optional<float> {
	        if (!std::isnan(data->bounds_min) &&
	            status_.position < data->bounds_min) {   // Sửa ở đây
	          return data->bounds_min;
	        }
	        if (!std::isnan(data->bounds_max) &&
	            status_.position > data->bounds_max) {   // Sửa ở đây
	          return data->bounds_max;
	        }
	        return {};
	      }();
